...
c outputs: F1, F2 (real*8) are structure functions per nucleus
      ...
      REAL*8 F1ff,F2ff,GMoff,GEoff,moff,xvalc(100),B,xm,m,BL,cof,qvmax
      REAL*8 g1,g2,g3,d1
...
c         if(IA.EQ.12) Esmin = 0.010
         if(IA.EQ.27) Esmin = 0.010       
         if(IA.EQ.40) Esmin = 0.010
         kf = xvalc(23)
c         kf = xvalc(23)+xvalc(40)*(A/12*sqrt(A/12.0)-1.0)
...
c         endif
         ...
         Es = Esmin
         if(qv.LT.qvmax) then
           Es = Es + xvalc(24)  !!! Maximum Es
c           Es = Es-xvalc(24)*(1.0-qv/qvmax)**xvalc(26) !!!  test  !!!            
           Es = Es-xvalc(24)*(1.0-(qv/qvmax)**2)**0.5 !!!  test  !!!
         endif
      endif 
      nup = nu-Es
...
CCMEC - Fitted Superscaling distribution      CCCC
     ...
     >  (1.0+abs(psip)/psimax)**2.0/(1.0+0.125*psip**2)**1


      FYn = xvalc(7)/ (1. + xvalc(8)**2 * (psipn + xvalc(9))**2) / 
     > (1. + exp(xvalc(10) * psipn))*(1.0-abs(psipn)/psimax)**2.0*
     > (1.0+abs(psipn)/psimax)**2.0/(1.0+0.125*psipn**2)**1
