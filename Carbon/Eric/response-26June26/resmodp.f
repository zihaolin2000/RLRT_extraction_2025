...
CCC  Version from Feb 22, 2025  -  Author:  M.E. Christy                    CCC
...
CCC  replaced 2-pi threshold with eta                                       CCC
      ...
      REAL*8 pgam(7),pwid(7,3),x0(7),dip,mon,q20,A0,t
      ...
      q20 = 0.05
c      q20 = xval(50)*(1.0+0.15*q20/(q20+q2))**3.0  !!! 9411
      q20 = xval(50)*(1.0+0.25*q20/(q20+q2))**2.0  !!! 9410      
c      q20 = xval(50)

      t = log(log((q2+q20)/0.25**2.0)/log(q20/0.25**2.0))
...
c         x0(i) = 0.14
      enddo
c      if(i.EQ.7) x0(i) = 0.15
      

c      if(sf.EQ.2) x0(1) = 0.08   !!!  different Delta mass for sigL
      if(sf.EQ.2) x0(1) = 0.015   !!!  different Delta mass for sigL      
      do i=1,7
        br(i,2) = 1.-br(i,1)-br(i,3)
      enddo
               
      dip = 1./(1.+q2/0.75)**2.0 !!!  Dipole parameterization  !!!
...
c     &          mon**rescoef(i,4)
     ...
     &          mon**rescoef(i,4)
          ...
          if(i.LE.6) height(i) = height(i)/(1.0+0.09*q2)**2.0
c          if(i.LE.6) height(i) = height(i)/(1.0+0.10*q2)
        else
...
c         height(7) = xval(49)/(1.0+0.5*q2)*mon**xval(45)
         height(7) = xval(49)/(1.0+0.03*q2)*dip**xval(45)
c         height(7) = xval(49)*dip**xval(45)
...
CCC    Finish resonances / start non-res background calculation   CCC
      ...
      if(sf.EQ.1.and.xpr(1).LT.1.0) then
         ...
         A0 = xval(37)/(1.0+q2/xval(42))**1.0

c         A0 = xval(37)/(q2+xval(42))
...
c         t1 = xval(38)*log(1.1+q2)+xval(39)/log(1.05+q2**0.5)

         t1 = xval(38)+xval(39)*t**xval(44)
c         t2 = xval(40) + xval(41)*log(1.1+q2)
c         t2 = xval(40)*(1.0+q2/xval(41))**xval(44)

         t2 = xval(40)+xval(41)*t**xval(43)
         
c         t2 = xval(40)*log(1.0+q2/xval(41))
...
c         A0 = xval(37)/(1.0+q2/xval(39))**2*log(2.72+xval(43)*q2*q2)
         A0 = xval(37)/(1.0+q2/xval(39))**2.0


         
c         t1 = xval(38)/(1.0+q2/(xval(40)))+xval(42)*log(2.72+q2)
c         t1 = xval(38)+xval(40)*t**xval(42)
         t1 = xval(38)/(1.0+xval(40)*t**xval(42))

         
c         t1 = xval(38)*
c     &          (1.+xval(40)*q2/(1.+0.1*q2))*
c     &          mon**xval(42)
c         if(q2.LT.0.1) t1 = 10.0
         
         t2 = xval(41)/(1.0+xval(43)*t)**2.0
         
c         t2 = xval(41)/(1.00+q2/xval(42))**xval(43)       !!! exponent of xpr

c         t2 = xval(41)+xval(42)/(1.00+q2)**2.0
...
c      if(sf.EQ.1) write(6,*) q2,t,t1,t2
